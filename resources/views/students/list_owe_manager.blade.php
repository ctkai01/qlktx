                        ...
                        <tbody class="table-border-bottom-0">
                            @foreach ($roomOwe as $index => $bill)
                                <tr>
                                    ...
                                    <td style="text-align: center">
                                        <div class='price-room'>{{$bill->TienPhong}}</div>
                                    </td>
                                    <td style="text-align: center">
                                        @if ($bill->DaThanhToan) 
                                    ...
                                    <td style="text-align: center">
                                        <a class="btn-payment" href="{{route('room_payment', $bill->id)}}"><button type="button" class="btn btn-success" aria-expanded="false">Thanh toán</button></a>
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
    ...
    <script>
        ...
        $(document).ready(function() {
            $('body').on('click', '.btn-payment', function(e) {
                ...
                swal({
                    title: 'Bạn có chắc ?',
                    text: 'Bạn có chắc muốn thanh toán tiền phòng tháng này ?',
                    icon: "warning",
                    buttons: true,
                    dangerMode: true,
                }).then((result) => {
